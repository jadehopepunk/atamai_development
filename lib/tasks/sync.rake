namespace :sync do
  ...
  task :production => [:environment] do
    ...
    echo_and_run "ssh #{username}@#{db_host} \"./dump_atamai_development.sh\""
    echo_and_run "rsync -az --progress #{username}@#{db_host}:~/dump.sql ./tmp/production_data.sql"
    echo_and_run "rsync -az --progress #{username}@#{db_host}:~/atamai_development/production/shared/system/* ./public/system/"
